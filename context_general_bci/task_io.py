...
class ShuffleInfill(RatePrediction):
    ...
    def forward(self, batch: Dict[str, torch.Tensor], backbone_features: torch.Tensor, compute_metrics=True, eval_mode=False, phase='train') -> torch.Tensor:
        ...
        # print(loss.size(), loss_mask.size(), loss.sum(), loss_mask.sum())
        loss = loss[loss_mask]
        batch_out['loss'] = loss.mean()
        if self.joint_heldout:
...
# def _int_edit_distance_update(
#     preds: Union[Union[str, Sequence[int]], Sequence[Union[str, Sequence[int]]]],
#     target: Union[Union[str, Sequence[int]], Sequence[Union[str, Sequence[int]]]],
#     substitution_cost: int = 1,
# ) -> torch.Tensor:
#     breakpoint()
#     # if isinstance(preds, str) or isinstance(preds, int):
#         # preds = [preds]
#     # if isinstance(target, str) or isinstance(target, int):
#         # target = [target]
#     # if not all(isinstance(x, str) or isinstance(x, Sequence[int]) for x in preds):
#     #     raise ValueError(f"Expected all values in argument `preds` to be string type, but got {preds}")
#     # if not all(isinstance(x, str) or isinstance(x, Sequence[int]) for x in target):
...
# class IntEditDistance(EditDistance):
#     r"""
#         Simple override of torchmetrics that removes the str typecheck because Levenshtein is not string-specific.
#     """
#     def update(self, preds: Union[str, Sequence[str]], target: Union[str, Sequence[str]]) -> None:
#         """Update state with predictions and targets."""
#         distance = _int_edit_distance_update(preds, target, self.substitution_cost)
