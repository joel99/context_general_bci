...
@dataclass
class DeoConfig(ExperimentalConfig):
    chop_size_ms: int = 1000

@dataclass
class CSTConfig(ExperimentalConfig):
    chop_size_ms: int = 1000
...
class DatasetConfig:
    ...
    merge_bin_factor: int = 1 # For extremely long data that can't be chopped (e.g. Seq2Seq, H2), subsmapling is used.
    ...
    # list of augmentations during dataloading. Names specified by dictionary key in augmentations.py
    ...
    max_arrays: int = 1
    behavior_dim: int = 2
    behavior_classes: int = 32 # 31 + 1 for padding for FALCON H2, note phoneme decoding (Brain2Text) 40 + 1.
    ...
    deo: DeoConfig = field(default_factory=DeoConfig)
    ...
    falcon_m1: FalconConfig = field(default_factory=FalconConfig)
    ...
    ndt3_parity_per_session_eval_split_continuous: bool = False
